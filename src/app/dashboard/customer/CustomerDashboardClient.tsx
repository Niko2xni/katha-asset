            ...
            {purchases.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {purchases.map((purchase) => (
                        <Card key={purchase.id} className="overflow-hidden bg-white border border-neutral-200 flex flex-col justify-between shadow-sm">
                            <div>
                                ...
                                <CardHeader className="p-6">
                                    ...
                                    <span className="text-[1.2rem] text-neutral-400">
                                        Purchased on: {new Date(purchase.createdAt).toLocaleDateString("en-PH", {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric'
                                        })}
                            ...
                            <CardFooter className="p-6 border-t border-neutral-100 bg-neutral-50/50 flex flex-col gap-3">
                                <div className="flex w-full items-center justify-between">
                                    ...
                                    <span className="text-[1.8rem] font-black text-neutral-900">
                                        {formatPrice(purchase.product.pricePhpInCents, purchase.product.priceUsdInCents)}
                                    </span>
                                </div>
            ...
            {selectedInvoice && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-[600px] border border-neutral-200 overflow-hidden relative flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-12 overflow-y-auto print:p-0 flex-1 space-y-8" id="printable-invoice">
                            {/* Header */}
                            <div className="flex justify-between items-start border-b border-neutral-100 pb-6">
                                <div>
                                    ...
                                    <p className="text-[1.1rem] text-neutral-400 mt-1">Digital Asset Trade Portal</p>
                                </div>
                                <div className="text-right">
                                    <span className="text-[1.2rem] font-bold bg-neutral-100 text-neutral-800 px-3 py-1 rounded">
                                        OFFICIAL INVOICE
                                    </span>
                                    <p className="text-[1.2rem] text-neutral-500 mt-2 font-mono">INV-{selectedInvoice.id.slice(0, 8).toUpperCase()}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6 text-[1.3rem]">
                                <div>
                                    <span className="text-neutral-400 font-semibold uppercase block">Billed To</span>
                                    <span className="font-bold text-neutral-800 mt-1 block">{userName}</span>
                                    <span className="text-neutral-500 block">{userEmail}</span>
                                </div>
                                <div className="text-right">
                                    <span className="text-neutral-400 font-semibold uppercase block">Transaction Date</span>
                                    <span className="font-bold text-neutral-800 mt-1 block">
                                        {new Date(selectedInvoice.createdAt).toLocaleDateString("en-PH", {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric',
                                            hour: '2-digit',
                                    ...
                                    <span className="text-neutral-500 block">Payment Method: PayMongo Gateway</span>
                                </div>
                            </div>
                            ...
                            <div className="border border-neutral-200 rounded-lg overflow-hidden">
                                <table className="w-full text-left text-[1.3rem]">
                                    <thead className="bg-neutral-50 border-b border-neutral-200">
                                        <tr>
                                            <th className="p-4 font-bold text-neutral-600">Product / Development Resource</th>
                                            <th className="p-4 font-bold text-neutral-600 text-right">Price</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-neutral-200">
                                        <tr>
                                            <td className="p-4">
                                                <span className="font-bold text-neutral-800 block">{selectedInvoice.product.title}</span>
                                                <span className="text-[1.1rem] text-neutral-400 block">Developed by {selectedInvoice.product.creator.name || "Anonymous"}</span>
                                            </td>
                                            <td className="p-4 text-right font-bold text-neutral-800">
                                                {formatPrice(selectedInvoice.product.pricePhpInCents, selectedInvoice.product.priceUsdInCents)}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <div className="border-t border-neutral-100 pt-6 flex flex-col items-end gap-2 text-[1.3rem]">
                                <div className="flex w-full max-w-[280px] justify-between">
                                    <span className="text-neutral-500">Net Amount (VAT-Exempt):</span>
                                    <span className="text-neutral-800 font-semibold">
                                        {formatPrice(
                                            Math.round(selectedInvoice.product.pricePhpInCents / 1.12),
                                            Math.round(selectedInvoice.product.priceUsdInCents / 1.12)
                                        )}
                                    </span>
                                </div>
                                <div className="flex w-full max-w-[280px] justify-between">
                                    <span className="text-neutral-500">Value Added Tax (12%):</span>
                                    <span className="text-neutral-800 font-semibold">
                                        {formatPrice(
                                            selectedInvoice.product.pricePhpInCents - Math.round(selectedInvoice.product.pricePhpInCents / 1.12),
                                            selectedInvoice.product.priceUsdInCents - Math.round(selectedInvoice.product.priceUsdInCents / 1.12)
                                        )}
                                    </span>
                                </div>
                                <div className="flex w-full max-w-[280px] justify-between border-t border-neutral-200 pt-3">
                                    <span className="text-neutral-900 font-bold">Total Settled:</span>
                            ...
                            <div className="text-center text-[1.1rem] text-neutral-400 border-t border-neutral-100 pt-6">
                                Thank you for your business. This receipt is automatically generated and acts as proof of purchase for digital development assets.
                            </div>
                        </div>
