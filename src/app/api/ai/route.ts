...
const localizationSchema = z.object({
  globalTitle: z
    .string()
    .describe("Cleaned, globally accessible SEO title wrapper."),
  englishTranslation: z
    .string()
    .describe(
      "A full translation or clean English alternative of any localized slang terms used.",
    ),
  globalKeywords: z
    .array(z.string())
    .describe(
      "An array of 6-8 search terms matching universal design jargon related to this asset.",
    ),
  suggestedCategories: z
    .array(z.string())
    .describe(
      "Top 2 standard architectural categories for indexing this asset.",
    ),
});
  ...
  try {
    ...
    if (session.user.role !== "CREATOR") {
      return new NextResponse(
        "Forbidden Resource: Creator credentials mandated.",
        { status: 403 },
      );
    }
    ...
    try {
      ...
      clearTimeout(timeoutId);
      console.warn(
        "AI Generation Fault/Timeout. Activating Graceful Degradation Routine:",
        aiError,
      );
