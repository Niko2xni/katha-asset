...
export async function POST(req: Request) {
  try {
    ...
    if (!signatureHeader || !process.env.PAYMONGO_WEBHOOK_SECRET) {
      return new NextResponse("Security Verification Context Deficit", {
        status: 400,
      });
    }
    ...
    if (!timestamp || !testSignature) {
      return new NextResponse("Malformed Signature Envelope Structure", {
        status: 400,
      });
    }
    ...
    if (calculatedBuffer.length !== testBuffer.length) {
      return new NextResponse(
        "Cryptographic Validation Integrity Discrepancy",
        { status: 401 },
      );
    }
    ...
    if (eventType === "checkout_session.payment.paid") {
      ...
      if (!metadata) {
        return new NextResponse("Target Session Metadata Context Absent", {
          status: 400,
        });
      }
      ...
      if (existingOrder && existingOrder.status === "PAID") {
        return new NextResponse(
          "Event Already Reconciled (Idempotent Intercepted)",
          { status: 200 },
        );
      }
    ...
    return new NextResponse("PayMongo Hook Reconciled Successfully", {
      status: 200,
    });
  } catch (error) {
    console.error("PayMongo Webhook Processing Defect:", error);
    return new NextResponse("Internal Server Error Processing Callback", {
      status: 500,
    });
  }
}
