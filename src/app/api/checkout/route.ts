...
async function fetchWithRetry(
  ...
  throw new Error(
    "Maximum retry limit exceeded for external gateway connectivity.",
  );
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!session?.user) {
      return new NextResponse("Unauthorized Execution Attempt", {
        status: 401,
      });
    }
    ...
    const targetPrice = isPhp
      ? product.pricePhpInCents
      : product.priceUsdInCents;
    ...
    if (!response.ok) {
      ...
      return new NextResponse("External Provider Rejection Exception", {
        status: 502,
      });
    }
    ...
    return new NextResponse(
      "Checkout Gateway Unavailable. Please try again later.",
      { status: 503 },
    );
  }
}
