...
export async function GET(req: Request, { params }: RouteParams) {
  try {
    ...
    if (!session?.user) {
      return new NextResponse("Unauthorized Download Attempt denied.", {
        status: 401,
      });
    }
    ...
    if (!product) {
      return new NextResponse("Target asset resource does not exist.", {
        status: 404,
      });
    }
    ...
    if (!userOwnsAsset) {
      return new NextResponse(
        "Access Denied: Missing verified payment authorization row.",
        { status: 403 },
      );
    }
    ...
    if (error || !data?.signedUrl) {
      ...
      return new NextResponse(
        "External storage cluster link assembly failed.",
        { status: 500 },
      );
    }
    ...
    return new NextResponse(
      "Internal Server Error during distribution routine.",
      { status: 500 },
    );
  }
}
