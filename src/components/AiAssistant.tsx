    ...
    const handleRefine = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert("AI processing failed. Please verify credentials or attempt again.");
            }
        } catch (e) {
            console.error(e);
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-[1.2rem] text-neutral-400 mt-1">
                        Translate Filipino design terms and optimize keywords for international storefront search algorithms.
                    </p>
                </div>

                <div className="space-y-4 text-[1.3rem]">
                    <div className="space-y-1">
                        <label className="text-neutral-400 font-bold block">Raw Localized Title</label>
                        <input
                            type="text"
                            value={rawTitle}
                    ...
                    <div className="space-y-1">
                        <label className="text-neutral-400 font-bold block">Raw Localized Description</label>
                        <textarea
                    ...
                    >
                        {loading ? "Optimizing Metadata..." : "✦ Refine & Optimize Metadata"}
                    </Button>
                </div>
                ...
                {result && (
                    <div className="bg-neutral-950 border border-neutral-800 rounded-lg p-5 space-y-4 text-[1.3rem] animate-in fade-in duration-300">
                        <div>
                            <span className="text-[1.1rem] font-bold text-emerald-400 uppercase tracking-wider block">SEO Optimized Title</span>
                            <p className="font-bold text-neutral-100 mt-0.5">{result.globalTitle}</p>
                        </div>
                        <div>
                            <span className="text-[1.1rem] font-bold text-emerald-400 uppercase tracking-wider block">English Translation & Expand</span>
                            <p className="text-neutral-300 mt-0.5 leading-relaxed">{result.englishTranslation}</p>
                        </div>
                        <div>
                            <span className="text-[1.1rem] font-bold text-emerald-400 uppercase tracking-wider block">Generated Jargon Jumps / Tags</span>
                            <div className="flex flex-wrap gap-2 mt-1.5">
                                {result.globalKeywords.map(k => (
                                    <span key={k} className="text-[1.1rem] bg-neutral-900 border border-neutral-800 text-neutral-400 px-2 py-0.5 rounded font-mono">
                                        #{k}
                                    </span>
                                ))}
                                {result.suggestedCategories.map(c => (
                                    <span key={c} className="text-[1.1rem] bg-emerald-950/50 border border-emerald-900/50 text-emerald-400 px-2 py-0.5 rounded font-mono">
                                        #{c}
                                    </span>
                                ))}
                            </div>
